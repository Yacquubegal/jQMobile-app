...
// on page load, fill in arrays from server
$(document).on("pagebeforeshow", "#viewReview", function (event) {   // have to use jQuery 
    FillUserArrayFromServer();
    FillReviewArrayFromServer();
});

$(document).on("pagebeforeshow", "#LoginOrRegister", function (event) {   // have to use jQuery 
    FillUserArrayFromServer();
});
$(document).on("pagebeforeshow", "#leaveReview", function (event) {   // have to use jQuery 
    FillUserArrayFromServer();
    FillReviewArrayFromServer();
});
...
function registerMe() {
    ...
    document.getElementById("homeResponse").innerHTML = x;
    addNewUser(new userObject(ID, name, password));
    FillUserArrayFromServer();
}
...
function submitReview() {
    ...
    addNewReview(new reviewObject(ID, userID, type, title, score, comments));
    FillReviewArrayFromServer();
}
...
function FillUserArrayFromServer(){
...
function FillReviewArrayFromServer(){
    // using fetch call to communicate with node server to get all data
    fetch('/reviewList')
    .then(function (theResonsePromise) {  // wait for reply.  Note this one uses a normal function, not an => function
        return theResonsePromise.json();
    ...
    .then(function (serverData) { // now wait for the 2nd promise, which is when data has finished being returned to client
    console.log(serverData);
    reviewArray.length = 0;  // clear array
    reviewArray = serverData;   // use our server json data which matches our objects in the array perfectly
    })
    .catch(function (err) {
     console.log(err);
    });
...
function addNewReview(newReview){
    // the required post body data is our movie object passed into this function
        ...
        const request = new Request('/addReview', {
            method: 'POST',
